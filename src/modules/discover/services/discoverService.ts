    ...
    async fetchAllData(userId?: string) {
        try {
            ...
            const copyRelations = userId ? 
                await fetch(`${JSON_SERVER_URL}/copyRelationships?copierId=${userId}`).then(res => res.json())
                : [];
            ...
            const processedStrategies = strategiesData.map(strategy => {
                ...
                const isCopying = copyRelations.some((rel: CopyRelationship) => rel.strategyId === strategy.id);
                ...
                return {
                    ...
                    isFollowing: currentUserData ? currentUserData.following.includes(strategy.leaderId) : false,
                    isCopying,
                };
            });
